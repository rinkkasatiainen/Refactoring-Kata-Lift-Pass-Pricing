...
def test_works_for_all_ages(lift_pass_pricing_app):
    response = requests.get(lift_pass_pricing_app + "/prices", params={'type': '1jour', 'age': 15})
    assert response.json() == {'cost': 35}


def test_works_for_night_passes(lift_pass_pricing_app):
    response = requests.get(lift_pass_pricing_app + "/prices", params={'type': 'night', 'age': 25})
